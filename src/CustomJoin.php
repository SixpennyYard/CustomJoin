...
class CustomJoin extends PluginBase implements Listener {
    ...
    "Welcome-Message" => "Bienvenue à toi §3{jplayer}§r sur §5Server !"
    ];
    ...
    public function onJoin(PlayerJoinEvent $event) {
        ...
        if($player->hasPlayedBefore()){
            ...
            if ($this->getConfig()->get("Message-Popup" == "false")) {
                $msg = $this->getConfig()->get("Message-Join");
                $msg = str_replace("{break}", "\n", $msg);
                $msg = str_replace("{player}", $player->getName(), $msg);
                $this->getServer()->broadcastMessage($msg);
            }elseif ($this->getConfig()->get("Message-Popup" == "true")) {
                $msg = $this->getConfig()->get("Message-Join");
                $msg = str_replace("{break}", "\n", $msg);
                $msg = str_replace("{player}", $player->getName(), $msg);
                $this->getServer()->broadcastPopup($msg);
            }elseif ($this->getConfig()->get("Message-Popup" == "true" || "false")) {
                $this->getServer()->getLogger()->error("Error in CustomJoin config: \"Join-Popup\" isn't set or true or false !");
